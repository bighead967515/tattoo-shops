...
var requestsRouter = router({
  ...
  getOpen: publicProcedure.input(
    ...
    }).from(tattooRequests).innerJoin(clients, eq2(tattooRequests.clientId, clients.id)).where(eq2(tattooRequests.status, "open")).orderBy(desc2(tattooRequests.createdAt)).limit(filters.limit ?? 20).offset(filters.offset ?? 0);
  ...
  listForHomepage: publicProcedure.query(async () => {
    ...
    }).from(tattooRequests).innerJoin(clients, eq2(tattooRequests.clientId, clients.id)).where(eq2(tattooRequests.status, "open")).orderBy(desc2(tattooRequests.createdAt)).limit(8);
    return results.map((r) => ({
      ...r.request,
      client: r.client,
      images: r.images ? JSON.parse(r.images) : [],
  ...
  getById: publicProcedure.input(z3.object({ id: z3.number() })).query(async ({ input }) => {
    ...
    const [result] = await db.select({
      request: tattooRequests,
      client: clients
    }).from(tattooRequests).innerJoin(clients, eq2(tattooRequests.clientId, clients.id)).where(eq2(tattooRequests.id, input.id)).limit(1);
    if (!result) {
      throw new TRPCError2({
        code: "NOT_FOUND",
...
var bidsRouter = router({
  ...
  getMyBids: protectedProcedure.query(async ({ ctx }) => {
    ...
    const myBids = await db.select({
      ...
      request: tattooRequests,
      client: clients
    }).from(bids).innerJoin(tattooRequests, eq2(bids.requestId, tattooRequests.id)).innerJoin(clients, eq2(tattooRequests.clientId, clients.id)).where(eq2(bids.artistId, artist.id)).orderBy(desc2(bids.createdAt));
    return myBids.map((b) => ({
      ...b.bid,
      request: b.request,
      client: b.client
