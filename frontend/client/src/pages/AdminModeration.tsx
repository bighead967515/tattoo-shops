            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 space-y-3">
                ...
                {doc.ocrProcessedAt ? (
                  <div className="bg-muted/50 rounded-lg p-4 space-y-2">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-medium">AI OCR Analysis</span>
                      {ocrVerdict === "verified" && (
                        <Badge className="bg-green-600 text-white">
                          <CheckCircle2 className="w-3 h-3 mr-1" /> Auto-Verified
                        </Badge>
                      )}
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-sm">
                      ...
                      <div>
                        ...
                        <span className="font-medium">{doc.ocrDocumentType || "—"}</span>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <span className={`font-medium ${ocrConfidence >= 70 ? "text-green-600" : ocrConfidence >= 40 ? "text-yellow-600" : "text-red-600"}`}>
                          {ocrConfidence}%
                        </span>
                      </div>
                    </div>
                    ...
                    {doc.ocrVerdictReason && (
                      <p className="text-xs text-muted-foreground mt-2 italic">{doc.ocrVerdictReason}</p>
                    )}

                    {ocrIssues.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {ocrIssues.map((issue) => (
                          <Badge key={issue} variant="outline" className="text-xs text-red-600 border-red-300">
                            {issue}
                          </Badge>
                        ))}
                      </div>
                ...
                <div className="flex items-center gap-4 text-xs text-muted-foreground">
                  ...
                  <span>Submitted: {new Date(doc.submittedAt).toLocaleDateString()}</span>
                </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-muted-foreground">by {item.userName || "Anonymous"}</span>
                  <span className="text-sm text-muted-foreground">→ {item.artistName || "Unknown Artist"}</span>
                ...
                {flags.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {flags.map((flag) => (
                      <Badge key={flag} variant="outline" className="text-xs text-orange-600 border-orange-300">
                        {flag}
                      </Badge>
                    ))}
                  </div>
              ...
              <div className="flex flex-col gap-2 shrink-0">
                ...
                >
                  <RefreshCcw className={`w-4 h-4 mr-1 ${reanalyzeMutation.isPending ? "animate-spin" : ""}`} />
                  Re-analyze
                </Button>
              </div>
            </div>
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Review flagged content and verify artist licenses with AI assistance.
            </p>
          </div>
        </div>
