        ...
        <section className="py-20 bg-gradient-to-b from-primary/5 to-transparent">
          <div className="container text-center">
            ...
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-12">
              From a free basic listing to a full-feature professional plan, we have the right tools to help you grow your client base and manage your business.
            </p>
          </div>
        </section>
              ...
              return (
                <Card 
                  ...
                  className={cn(
                    ...
                    isMostPopular ? "border-2 border-primary bg-gradient-to-br from-primary/10 to-background" : "border-border"
                  )}
                >
                  {isMostPopular && (
                  ...
                  <div className="space-y-4 text-sm">
                    <div className="flex items-center gap-3">
                      {limits.portfolioPhotos === Number.MAX_SAFE_INTEGER ? <Check className="w-5 h-5 text-green-500"/> : <Check className="w-5 h-5"/>}
                      <span><b>{limits.portfolioPhotos === Number.MAX_SAFE_INTEGER ? 'Unlimited' : limits.portfolioPhotos}</b> Portfolio Photos</span>
                    </div>
                     <div className="flex items-center gap-3">
                      {limits.canAcceptBookings ? <Check className="w-5 h-5 text-green-500"/> : <X className="w-5 h-5 text-destructive"/>}
                      <span>Accept Bookings</span>
                    </div>
                     <div className="flex items-center gap-3">
                      {limits.canShowDirectContact ? <Check className="w-5 h-5 text-green-500"/> : <X className="w-5 h-5 text-destructive"/>}
                      <span>Show Direct Contact</span>
                    </div>
                     <div className="flex items-center gap-3">
                      {limits.canRespondToReviews ? <Check className="w-5 h-5 text-green-500"/> : <X className="w-5 h-5 text-destructive"/>}
                      <span>Respond to Reviews</span>
                    </div>
                     <div className="flex items-center gap-3">
                      {limits.hasAnalytics ? <Check className="w-5 h-5 text-green-500"/> : <X className="w-5 h-5 text-destructive"/>}
                      <span>Profile Analytics</span>
                    </div>
                     <div className="flex items-center gap-3">
                      {limits.isFeatured ? <Check className="w-5 h-5 text-green-500"/> : <X className="w-5 h-5 text-destructive"/>}
